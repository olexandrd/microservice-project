...
Note: the Helm chart currently uses `POSTGRES_NAME`, while Django settings read `POSTGRES_DB`. Align these names before relying on the chart-provided database name. Postgress password is stored on env variables.
